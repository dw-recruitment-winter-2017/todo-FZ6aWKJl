...
(defn ttl-memory-store
  ...
  in seconds.  By default, the returned session store is based on expiring-map. Expiring-map
  options (like listeners and expiration policy) can be given as an optional map. If an 
  optional key :core-cache is given, a session stored based on core.cache is returned."
  [ttl & [opts]]
  (ExpiringMapStore. (em/expiring-map ttl (merge opts {:expiration-policy :access}))))
